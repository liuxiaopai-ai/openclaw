...
- Plugins/context-engine startup: wait for async plugin registration before gateway startup hooks run, so `boot-md` can resolve third-party context engines during startup instead of failing early with `Context engine ... is not registered`. (#39725)
